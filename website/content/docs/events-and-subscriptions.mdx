...
listeners on the node's **direct child nodes** (deeper descendants keep
theirs — clear them per node, or hold on to the unsubscribe functions
returned by `Retree.on`):

```ts
Retree.clearListeners(tree.lists);        // clear the lists node's own listeners
Retree.clearListeners(tree.lists, false); // also clear each list's listeners
```
