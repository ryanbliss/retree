...
value arrives, the query node restores the last clean server value. If Convex
sends a changed value first, the dirty optimistic state is cleared and a later
...
`ConvexNode` then disposes the live query, paginated query, and
connection-state children created through its helper methods — no manual
cleanup effect needed for constructor-created queries. Calling `dispose()`
yourself is still useful for non-React app shutdown.
...
status), `error`, `updateArgs(...)`, and `dispose()`.
