...
`useSelect` subscriptions are *observational*: when a selected dependency
changes, this component re-renders, but the node you passed in never emits or
reproxies — other subscribers see no change. When the owner node itself
should emit `nodeChanged` for a selection, that's the `@select` decorator's
job. The full distinction, with a comparison table, lives in
