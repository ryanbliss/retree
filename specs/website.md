...
-   `/docs` — journey-ordered:
    ...
    -   **Events & subscriptions (core)** — `@retreejs/core` outside React: `Retree.root/on/select`, the three event types, `INodeFieldChanges` records (raw previous/new + `Retree.managed`), unsubscribe patterns. The Convex guide links here.
...
**Trade-offs / "when not to use Retree" section on /why** — addresses head-on what a skeptical senior engineer will raise: (1) all hooks use `useSyncExternalStore`, while live mutable proxies still do not provide immutable historical state or transition-scheduled mutations; (2) React Compiler interaction (the site itself runs react-compiler — say so); (3) the per-node subscription model means subscribing to the right node is on you (the whiteboard example — framed as explicit granularity, taught in Thinking in Retree); (4) project status: solo maintainer, v0.6.x, versioning policy, test count, open benchmark harness. Recommend Zustand outright for small flat stores; credit MobX's decade of hardening + best-in-class computed engine; credit Valtio's minimalism + Redux DevTools.
...
2. Design system: tokens (dark+light, `accent-text` ≥4.5:1 + `accent-glow`, contrast-validated **before page work**), no-flash theme script, fonts, nav/footer shells, MDX component map, Shiki setup. **Deliverable ✅ measured min+gzip (esbuild, externals react/react-dom, re-measured July 13 2026 after the external-store integration): `@retreejs/core` 17.1 kB gz; `core+react` 20.5 kB gz; `mobx` 19.1 kB gz; `mobx+mobx-react-lite` 21.5 kB gz; `valtio` 2.8 kB gz; `zustand` 0.4 kB gz. Ruling: no size claim in hero strip; comparison table states all numbers honestly ("undercuts the MobX stack by ~1.0 kB; Valtio/Zustand are far smaller — if minimal bytes matter most, use them").**
