...
Running `npm create @retreejs@latest` offers the compiler when a Babel config
is present and can add it to a JSON config for you.
...
Fields of a compiled node live on the raw object, so `Object.keys`, spread,
`JSON.stringify`, and `Object.hasOwn` on a managed node do not see them. Use
`Retree.raw(node)` for those. `delete node.field` and `Object.defineProperty`
on a managed node are not supported. Getters replaced on a live prototype
after the class was defined are not picked up; methods are.
...
node. Test spies work on methods (`vi.spyOn(node, "method")`); to spy on an
arrow-function field, spy on `Retree.raw(node)`.
