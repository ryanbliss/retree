    ...
    implements ProxyHandler<T>, ICustomProxyHandler<T>
{
    ...
    public getBoundFunction<TFunction extends Function>(
        prop: string | symbol,
        source: TFunction,
        thisArg: unknown
    ): TFunction {
    ...
    public get(target: T, prop: string | symbol, receiver: any): any {
        ...
        if (cachedChildren !== null && typeof prop === "string") {
            const child = cachedChildren[prop];
            if (child !== undefined) {
                return trackPropertyAccessIfNeeded(
                    this,
                    baseProxy,
    ...
    /**
     * A managed node stored as a proxy inside plain input (raw input is not
     * walked up front). Store its raw node in the target (raw purity) and
     * serve the managed child. A locked property keeps the stored proxy: the
     * get invariant requires the exact value for non-writable,
     * non-configurable data properties.
     */
    private adoptStoredProxy(
        target: T,
        prop: string | symbol,
        storedProxy: object,
        raw: object
        ...
        Reflect.defineProperty(target, prop, { ...descriptor, value: raw });
...
function buildProxyHandler<T extends TreeNode = TreeNode>(
    ...
    const existing = knownUnmanaged
        ? undefined
        : getBaseHandlerForUnproxiedNode(object);
    ...
    if (object instanceof Map) {
        ...
        for (const storedValue of object.values()) {
            if (storedValue === null || typeof storedValue !== "object") {
                continue;
            }
            const value = getUnproxiedNode(storedValue) ?? storedValue;
...
/** Builds the child edge for one stored field of a fresh object or ReactiveNode. */
function adoptStoredField(
    proxyHandler: BaseProxyHandler<TreeNode>,
    object: object,
    prop: string,
    emitter: TreeChangeEmitter
): void {
    const storedValue = (object as Record<string, unknown>)[prop];
    if (storedValue === null || typeof storedValue !== "object") {
        return;
    }
    ...
    // Raw purity: a proxy the constructor stored is replaced by its raw node
    // before the child edge is built.
    if (
        storedHandler !== undefined &&
...
function setProxiedChild(
    ...
    setProxiedChildHandler(
        proxyHandler,
        prop,
        getBaseHandlerOfProxy(childProxy)
    );
}

function setProxiedChildHandler(
    proxyHandler: ICustomProxyHandler<any>,
    prop: string | symbol,
    childHandler: BaseProxyHandler<TreeNode>
) {
    (proxyHandler[proxiedChildrenKey] ??= createChildrenCache())[prop] =
        childHandler;
}
...
/** Plain objects and arrays are proxied on first read; other shapes eagerly. */
function hasLazilyProxiedShape(value: object): boolean {
    return (
        Array.isArray(value) ||
...
function shouldCreatePlainObjectProxyLazily(value: unknown): value is object {
    ...
    // Cheap shape check first; isCustomProxy costs a sentinel trap read.
    if (!hasLazilyProxiedShape(value)) {
        return false;
    }
...
function getOrCreateProxiedChild(
    ...
    return getOrCreateProxiedChildHandler(proxyHandler, prop, value, emitter)
        .baseProxy;
}

function getOrCreateProxiedChildHandler(
    proxyHandler: ICustomProxyHandler<any>,
    prop: string | symbol,
    value: object,
    emitter: TreeChangeEmitter
...
type TArrayChildCacheSnapshot = (BaseProxyHandler<TreeNode> | undefined)[];
...
function snapshotArrayChildCache(
    handler: BaseProxyHandler<any>,
    length: number
): TArrayChildCacheSnapshot {
    const snapshot: TArrayChildCacheSnapshot = new Array(length);
...
function moveArrayChild(
    ...
    toIndex: number,
    baseProxy: TCustomProxy<any>,
    cacheSnapshot: TArrayChildCacheSnapshot
): void {
    const toKey = String(toIndex);
    const entry = cacheSnapshot[fromIndex];
    if (entry !== undefined) {
        setProxiedChildHandler(handler, toKey, entry);
    } else {
        deleteProxiedChild(handler, toKey);
    }
    let childHandler = entry;
    if (childHandler === undefined) {
        ...
        childHandler = getBaseHandlerForUnproxiedNode(rawValue);
    }
    if (childHandler === undefined) {
        return;
    }
...
function rebuildArrayChildrenAfterReorder(
    ...
    const entryByRaw = new Map<object, BaseProxyHandler<TreeNode>>();
    ...
    for (let i = rawBefore.length - 1; i >= 0; i--) {
        ...
        const entry = cacheSnapshot[i];
        if (raw !== null && typeof raw === "object" && entry !== undefined) {
            entryByRaw.set(raw, entry);
        }
    }
    for (let i = 0; i < target.length; i++) {
        ...
        const entry =
            entryByRaw.get(raw) ?? getBaseHandlerForUnproxiedNode(raw);
        if (entry === undefined) {
            deleteProxiedChild(handler, key);
            continue;
        }
        setProxiedChildHandler(handler, key, entry);
    ...
    if (prop === "copyWithin") {
        return function copyWithinWrapper(
            ...
            for (let offset = 0; offset < count; offset++) {
                ...
                const entry =
                    cacheSnapshot[sourceIndex] ??
                    getBaseHandlerForUnproxiedNode(sourceRaw);
                if (entry === undefined) {
                    deleteProxiedChild(handler, destinationKey);
                    continue;
                }
                setProxiedChildHandler(handler, destinationKey, entry);
